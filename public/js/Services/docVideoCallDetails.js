...
function connect() {
  if ($("#txtname").val() == "") {
    easyrtc.showError("MEDIA-ERROR", "Please enter your name first");
    return false;
  }
...
function loginSuccess(easyrtcid) {
  ...
  document.getElementById("iam").innerHTML =
    "I am " + easyrtc.idToName(easyrtcid); // easyrtcid;
}
...
easyrtc.setStreamAcceptor(function (easyrtcid, stream) {
  console.log("this is id of accepted=" + easyrtcid);
...
easyrtc.setAcceptChecker(function (easyrtcid, callback) {
  ...
  if (easyrtc.getConnectionCount() > 0) {
    document.getElementById("acceptCallLabel").innerHTML =
      "Drop current call and accept new from " +
      easyrtc.idToName(easyrtcid) +
      " ?";
  } else {
    document.getElementById("acceptCallLabel").innerHTML =
      "Accept incoming call from " + easyrtc.idToName(easyrtcid) + " ?";
  }
