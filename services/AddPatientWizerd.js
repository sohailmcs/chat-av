...
$(function() {
                    ...
                    .then((data) => {
                        if (data > 0 && type.toLowerCase() == "call".toLowerCase()) {
                            ...
                            PatientDetailsInfo($("#hdnPatientId").val(), "Child")
                                .then((d) => {
                                    if ((pType == "Child" || pType == "Relative") && $("#dboInfo").val() == "0") {
    ...
    $(".previous").click(function() {
        ...
        if ($(this).attr("step") == "third")
            PatientBasicInfo($("#hdnPatientId").val(), pType);
...
function AddPatientMedication() {
    ...
    $.ajax({
        ...
        success: function(data, textStatus, xhr) {
            setCookie('MedicationIds', data.medicationIds, 1);
            setCookie('AlergyIds', data.alergyIds, 1);
...
function AddPatientConditon() {
    ...
    if ($("#rdoTakeMedNo").prop("checked")) {
        PatientMedCondition.push({
            ...
            MedConditionDesc: $("#otherCondition").prop("checked") == true ?
                $("#txtMedCondition").val() : "",
            AddedBy: userLoginId,
        });
    }
    ...
    $.ajax({
        ...
        success: function(data, textStatus, xhr) {
            ...
            setCookie('ComplinIds', data.complainId, 1);
            setCookie('MedCondition', data.medConditionid, 1);
